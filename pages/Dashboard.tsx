...
import { TrendingUp, Package, Users, DollarSign, Clock, Star, Trophy, Bike, AlertCircle } from 'lucide-react';
import { getCompanySummary, getCompanyRevenueChart, getCompanyDeliveryStats, getDeliveries, getDrivers } from '../services/api';
            ...
            {stats?.recent_deliveries?.map(d => (
              <div key={d.id} className="flex items-center justify-between text-sm py-2 border-b border-gray-50 last:border-0">
                <div className="min-w-0">
                  <p className="font-medium text-gray-900 truncate">#{d.id} - {d.customer?.name || 'Customer'}</p>
